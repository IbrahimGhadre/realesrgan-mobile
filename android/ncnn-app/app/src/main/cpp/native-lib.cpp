...
// NCNN model declaration
ncnn::Net realesrgan;

// Stores whether Vulkan is available (prevents multiple get_gpu_count calls)
bool isVulkanAvailable = false;

// Function to copy a file from assets to local storage
...
// Function to process an image with Real-ESRGAN
...
Java_com_example_realesrganappncnn_MainActivity_processImage(JNIEnv *env, jobject instance, jobject bitmap) {
    ...
    // Convert to OpenCV Mat (RGBA)
    cv::Mat rgba(info.height, info.width, CV_8UC4, pixels);
    cv::Mat rgb;
    cv::cvtColor(rgba, rgb, cv::COLOR_RGBA2RGB);
    AndroidBitmap_unlockPixels(env, bitmap);

    // Central CROP to make the image square
    ...
    // Simulate JPEG COMPRESSION: downscale + upscale
    ...
    // Convert ncnn::Mat to cv::Mat RGBA, then to BGR
...
Java_com_example_realesrganappncnn_MainActivity_preprocessImage(JNIEnv *env, jobject instance, jobject bitmap) {
    ...
    // Convert to ARGB_8888 Bitmap for display
    cv::Mat rgba_out;
